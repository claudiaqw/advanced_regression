...
Los resultados de cada uno de los modelos serán comparados en función del RMSE y nos quedaremos con el que menor error alcanza. Cada uno de los modelos será probado con todas las variables y con una elección resultado de aplicar selección de variables stepwise. Además, para la evaluación será utilizado el método de cross-validation.
...
ridge.complete <- train(lm.complete, data = train,
                    method='ridge',
...
ridge.wise <- train(wise.model, data = train,
                    method='ridge',
                    preProc=c('scale', 'center'),
                    tuneGrid = ridge_grid,
                    trControl=ctrl)
...
lasso.complete <- train(lm.complete, data = train,
                    method='lasso',
                    preProc=c('scale','center'),
                    tuneGrid = lasso_grid,
                    trControl=ctrl)
...
test_results$ridge.complete <- predict(ridge.complete, test)
test_results$ridge.wise <- predict(ridge_tune, test)
...
```{r}
tuneGrid <- expand.grid(alpha = 1, lambda = seq(210 ,220, length = 100))

lasso_tune <- train(Deaths~., 
                    data = train,
                    method = "glmnet",
                    tuneGrid = tuneGrid,
                    trControl = ctrl,
...
train_results <- predict(lasso_tune, train)
postResample(pred = train_results,  obs = train$Deaths)
```


```{r}
train_results <- predict(lasso_tune, data_exp)
postResample(pred = train_results,  obs = data_exp$SalePrice)
```
